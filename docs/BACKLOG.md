...
**2b ではやらない:** 欠配1回ごとの自動返金、期間延長 API。
**法務ページ（実装済み）:** `/legal/sct`・`/legal/privacy`・`/legal/terms`（`LEGAL_*` は Fly secrets / `.env`。リポジトリに個人情報を載せない）。
...
- [x] GHA 日次サマリー欠走: Fly **09:00 JST** に `summary_documents` 存在チェック、無いときだけ Discord（生成・メールしない）
