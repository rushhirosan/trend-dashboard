...
| 領域 | 状態 | 次のアクション |
|------|------|----------------|
| **課金入り口（2b）** | PAY.JP v2 実装済だが **本番審査不可**（Visa〜Apple Pay 全否）。Stripe も不可済 | **次:** 代替決済 or 媒体課金を決める（下記「本線ブロッカー」） |
...
**課金戦略（2026-09-24）:** Stripe・PAY.JP とも **本番審査不可**。自サイトクレカの本線はいったん止め、代替を選ぶ。v2 Checkout 実装・ローカル E2E・有料配信パイプラインは資産として残す。

### 本線ブロッカー — 決済再選定（いまここ）

| 候補 | 向き | メモ |
|------|------|------|
| **別 PSP（fincode / KOMOJU / Square 等）** | 自サイト課金を続けるなら本命候補 | 審査・商材（デジタル／定期メール）の通りやすさを先に確認。実装は adapter 差し替え |
| **note 有料（メンバーシップ等）** | 審査回避・最短でお金を取る | 以前は「課金箱にしない」方針だったが、両 PSP 不可後は再検討余地あり |
| **銀行振込／手動** | 検証用のみ | スケールしない。お試し人数が極少なら一時しのぎ |
| **課金を一旦止める** | dogfood 継続 | コンテンツ品質・配信安定を優先。2b は保留 |

PayPay は PAY.JP 側で未申請なら別結果メールは来ない想定。アカウントは「翌月以降順次利用不可」——テスト鍵・管理画面依存は早めに切り離す。

### 本線 — フェーズ2b 残り（決済確定後に再開）

| # | タスク | 状態 | やること |
|---|--------|------|----------|
| 1 | **有料配信 GHA 連携** | 済 | dogfood の後に `--from-api`。購読者 API は 2026-08-19 デプロイ済 |
| 2 | **PAY.JP v1 定期（クレカ）** | ローカル済・**本番不可** | 審査否。コード資産のみ |
| 3 | **お試し商品の定義** | 方針決定 | **ワンショット約30日 / ¥300〜500 / JP のみ / 自動更新なし**。プロバイダ差し替え後も流用可 |
| 4 | **PAY.JP API v2 都度払い** | 実装済・**本番不可** | Checkout v2 hosted。審査全ブランド不可のため live 接続しない |
| 5 | **セキュリティ申告・本番申請** | **否決（2026-09-24）** | Visa〜Discover / Apple Pay すべて不可。理由非開示 |
| 6 | **DB: お試し期限** | 実装済 | `expires_at` で配信対象から除外。期限後は再購入 |
| 7 | **Fly secrets + Webhook** | 保留 | live 鍵は入れない。代替 PSP 確定後に再設定 |
| 8 | **本番お試し購入 → 有料メール** | 未（決済待ち） | プロバイダ確定後に 1人確認 |

10人・継続率は入り口の条件ではない。お試しで温度を見てから月額自動を検討。

### 2b 最小セット — 進捗（実装チェック）

- [x] **決済（v1 定期）** — ローカル E2E 済（コード資産）。**本番審査不可**
- [x] **決済（v2 お試し）** — Checkout v2 実装済。**本番審査不可**（live 接続しない）
...
- [x] **UI** — 「お試し30日」+ Checkout リダイレクト（本番鍵なしでは販売不可）
- [ ] **決済プロバイダ再選定** — Stripe / PAY.JP 不可後の代替
- [ ] **有料メール受信確認** — 決済接続後に購入者1人 + GHA
...
- **決済:** Stripe・PAY.JP とも本番審査不可（2026-09-24）。**代替選定中。** 月額自動は後続。US は後続プロバイダ。v2 Checkout 実装は資産として残す
...
**法務ページ（実装済み）:** `/legal/privacy`・`/legal/terms` は公開。`/legal/sct`（特商法）はルート残置・`noindex`・ナビ/sitemap 非表示（課金再開まで。`LEGAL_*` は Fly secrets / `.env`）。
...
フェーズ2a  Fake door + Waitlist 実装 ← 実装済み。本番登録は開かない
    ↓
フェーズ2b  課金入り口 → 限定有料 ← **ブロッカー**（Stripe / PAY.JP 本番不可。代替選定中）
    ↓
...
- [x] PAY.JP v1 定期: ローカル E2E 済（**本番審査不可**）
- [x] お試し商品: 約30日 / ¥500 既定（`PAYJP_TRIAL_*`）/ JP / 自動更新なし
- [x] PAY.JP **API v2** 都度払い実装 + Webhook（**本番審査不可・live 接続しない**）
- [x] DB: `expires_at`（お試し期限）
- [x] UI: お試し購入 CTA（本番鍵なしでは販売不可）
- [x] セキュリティ申告・本番申請 → **2026-09-24 全ブランド否決**
- [ ] 代替決済 or 媒体課金を決定
- [ ] Fly secrets / Webhook（プロバイダ確定後）
- [ ] お試し購入1人で有料メール受信確認
- [ ] （後続）月額自動 + 継続率
...
| 2026-09-24 | **PAY.JP 本番審査否決**（Visa/MC/JCB/Amex/Diners/Discover/Apple Pay すべて不可。理由非開示）。アカウントは翌月以降順次利用不可。Stripe に続き自サイトクレカ不可。2b は代替決済 or 媒体課金の再選定へ。 |
