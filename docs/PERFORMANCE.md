...
| **Inter（US）** | 自前ホスト済み（2026-06-27） | デプロイ後 PSI `/us` 再計測 |
...
> **Mobile ~60 → 82** / **Desktop 89 → 97**（フェーズ2b まで完了）。**当面は新規の大規模パフォーマンス改修は行わず、定期計測のみ。**

---

## 運用方針（フェーズ3以降）

**2026-06-28 時点の判断:** フェーズ1〜2b で十分な改善（Mobile **82** / Desktop **97**）。フェーズ3（DOM 遅延レンダリング等）は **デグレリスクが高く費用対効果が低い** ため、**スコアが下がったときだけ** 再検討する。
...
|------|---------------------------|------|
| Mobile | **75 未満** | 2026-06-28 実績 82。~10 点以上の下落 |
| Desktop | **90 未満** | 2026-06-28 実績 97 |
...
フェーズ3  DOM 構造（非アクティブタブ遅延レンダリング）      ← 保留（スコア低下時のみ）
    ↓
フェーズ4  計測・運用（PSI 月次 / 低下時にフェーズ3検討）   ← いま
```
...
**いまの主タスク:** 下記を継続し、スコアが [運用方針](#運用方針フェーズ3以降) の警戒ラインを下回ったらフェーズ3以降を検討する。
...
`lighthouse-reports/` に `.report.html` / `.report.json` が出力される。HTML をブラウザで開き、Performance スコアと Opportunities / Diagnostics を確認する。
