...
**定期実行（スケジューラー）:**
- 深夜1:00（日本時間） → スケジューラー実行 → 全API呼び出し → キャッシュ更新 → メール自動送信
- 朝7:00（日本時間） → スケジューラー実行 → 全API呼び出し → キャッシュ更新 → メール自動送信
- 昼13:00（日本時間） → スケジューラー実行 → 全API呼び出し → キャッシュ更新 → メール自動送信
- 夜19:00（日本時間） → スケジューラー実行 → 全API呼び出し → キャッシュ更新 → メール自動送信
...
- データ更新はスケジューラー（1日4回：1時、7時、13時、19時 JST）と明示的な手動実行のみに依存
...
- ✅ スケジューラーによる自動更新（深夜1:00・朝7:00・昼13:00・夜19:00 日本時間）完了後に自動的にメール送信
