...
| ファイル | 内容 |
|----------|------|
| `YYYY-MM-DD.md` | OpenAI で生成した Markdown（成功時のみ上書き。**`generator: openai`** がフロントマターにあるのが AI 生成の目印）。構成は **急上昇3つ（機械）→ 複数ソース重なり（最大3・LLM）→ カテゴリ別トップ3（機械）**（観測日＝ファイル名、読者は通常翌朝に受け取る） |
