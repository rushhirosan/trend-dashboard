# 日次 X ツイート案 — 2026-05-27

`docs/x_post_samples/daily_guide.md` の **夜用・急上昇3つ** 型。**朝の読み物は AI 日次サマリー**（`docs/summaries/daily/`）のみ。

- **自動投入:** `scripts/generate_daily_x_post_series.py --write` が **`trend_daily_snapshots`** の **07 / 13 / 19** を読み、**全ソース横断で順位が上がったラベル**を最大3件（JP/US 各ブロック）選びます。入力は **`DATABASE_URL`** か **`--from-api`**（`GET /api/summaries/daily-snapshots`）。
- **GitHub Actions:** `.github/workflows/daily-x-post-series.yml` が **JST 20:10 前後（UTC 11:10）** に **`docs/x_post_samples/daily/{日付}.md`** を更新します。
- 一覧: https://trends-dashboard.fly.dev/
- 鮮度: https://trends-dashboard.fly.dev/data-status

**US 返信に足す場合（任意・英語）:**

```
Dashboard refreshes on a JST schedule (1/7/13/19 JST). Same post time as our JP tweet (8pm JST ≈ US morning).
```

---

## 2026-05-27

### JP — 今日の急上昇3つ

```
【2026-05-27】今日の急上昇3つ（JP）
① WR Twigに複数の脆弱性（IT）
② ボブ・ホーナーさん死去、６８歳 ヤクルトでも活躍、球界席巻―米大リーグ（ニュース）
③ なけなしの130万を使って、Cloud Praticaに人生をオールインした話（IT）
一覧: https://trends-dashboard.fly.dev/
```

### US — 今日の急上昇3つ（英語）

```
Today's rising 3 (US) 2026-05-27 · 8pm JST
① Houston Astros pitchers combine for first no-hitter in MLB… (News)
② AI Chatbot Recommendations Redirect Users to Cryptojacking… (IT)
③ Several killed after train hits school bus in Belgium, local… (News)
一覧: https://trends-dashboard.fly.dev/us
```
